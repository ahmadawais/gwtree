...
describe('createWorktree', () => {
  ...
  it('should create worktree with existing branch', async () => {
    ...
    vi.mocked(p.select).mockResolvedValueOnce('main').mockResolvedValueOnce('none');
  ...
  it('should create worktree with new branch', async () => {
    ...
    mockExistsSync.mockReturnValue(false);
    vi.mocked(p.select).mockResolvedValueOnce('new').mockResolvedValueOnce('none');
    vi.mocked(p.text).mockResolvedValueOnce('new-feature').mockResolvedValueOnce('test');
  ...
  it('should open VS Code when selected', async () => {
    ...
    mockExistsSync.mockReturnValue(false);
    vi.mocked(p.select).mockResolvedValueOnce('main').mockResolvedValueOnce('code');
    vi.mocked(p.text).mockResolvedValueOnce('test');

    await createWorktree();

    expect(mockExecSync).toHaveBeenCalledWith(expect.stringContaining('code'), expect.any(Object));
  });
  ...
  it('should open default editor when selected', async () => {
    ...
    mockExistsSync.mockReturnValue(false);
    vi.mocked(p.select).mockResolvedValueOnce('main').mockResolvedValueOnce('default');
    vi.mocked(p.text).mockResolvedValueOnce('test');
    ...
    expect(mockExecSync).toHaveBeenCalledWith(expect.stringContaining('nano'), expect.any(Object));
  });
